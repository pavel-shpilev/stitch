...
from taskboard.views import BoardList, BoardView, LabelView, MemberList, MemberView, \
    ColumnList, ColumnView, CardList, CardView
...
urlpatterns = [
    ...
    url(r'^columns/(?P<column_id>[0-9]+)/cards/$', CardList.as_view()),  # List+ create cards.
    url(r'^columns/(?P<pk>[0-9]+)/$', ColumnView.as_view()),  # Rename + archive + reorder.
    url(r'^cards/(?P<pk>[0-9]+)/$', CardView.as_view()),  # Rename + archive + reorder.
